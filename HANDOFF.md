...
## Phase 6 — DONE (intelligence autopilot + WhatsApp inbound log)

Deferred follow-ups remaining:
- **Client re-profiling cadence** — `profiler.reprofile_org_clients()` exists but is not on a Celery beat schedule yet. Add a weekly task when desired.
- **Timing-optimizer enforcement** — `next_send_window()` is exposed via the engine/preview API but the pipeline still uses contact-window + reschedule logic; wiring the optimizer's exact window into scheduling is optional polish.
