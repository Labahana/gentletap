...
- **VPS secret rotation (owner action, cannot be done from here):** rotate the Paddle webhook secret and JWT/token-encryption keys on the VPS + Paddle dashboard — they exist in git history.
...
- Optional future ideas: per-affiliate resource page, intelligence recommendations surfaced inside the app UI (endpoint already exists), email-reply tracking for `client_responded_recently` (currently WhatsApp inbound + replied-status only).
