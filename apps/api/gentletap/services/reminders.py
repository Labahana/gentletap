...
from gentletap.plans import has_unlimited_sequences, has_whatsapp
...
from gentletap.services.email_router import get_send_provider, has_delivery_capability, send_reminder_message
from gentletap.services.sequences import MAX_SEQUENCE_STEP, advance_sequence_after_send, schedule_next_job


def count_active_sequences(db: Session, user_id: UUID) -> int:
    return (
        db.query(Invoice)
        .filter(
...
def ensure_can_activate(db: Session, user: Profile, additional: int = 0) -> None:
    if has_unlimited_sequences(user.plan):
        return
    limit = get_settings().free_plan_active_invoice_limit
    active = count_active_sequences(db, user.id)
    if active + additional > limit:
        raise HTTPException(
            status_code=status.HTTP_402_PAYMENT_REQUIRED,
            detail=f"Starter plan allows {limit} active invoice sequences. Upgrade to Pro or higher.",
        )


def generate_draft(db: Session, invoice: Invoice) -> ReminderMessage:
...
def approve_all_overdue(db: Session, user: Profile) -> dict:
    if not has_delivery_capability(db, user.id, plan=user.plan):
        raise HTTPException(
            ...
            detail="Connect Gmail or verify an email sender before going live",
        )
    ...
    ensure_can_activate(db, user, additional=len(to_activate))
    ...
    return {"activated": activated, "message": f"Activated {activated} invoice sequences"}
...
def process_due_job(db: Session, job_id: UUID) -> None:
    ...
    if message is None and result.message:
        message = ReminderMessage(
            ...
            body=result.message.body,
            tone=result.tone.value if result.tone else None,
            channel=(result.channel or Channel.EMAIL).value,
            status="approved",
        )
        db.add(message)
        db.flush()
    ...
    if channel == Channel.EMAIL and (not invoice.client or not invoice.client.email):
        job.status = "failed"
        db.commit()
        return
    if channel == Channel.WHATSAPP and (not invoice.client or not invoice.client.phone):
        job.status = "failed"
        db.commit()
        return
    ...
    try:
        external_id = send_reminder_message(
            ...
            to_phone=invoice.client.phone if invoice.client else None,
        )
        message.status = "sent"
