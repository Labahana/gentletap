...
(Idempotent: safe on databases already initialized via create_all.)
...
def upgrade() -> None:
    ...
    if not _has_table("client_profiles"):
        op.create_table(
            "client_profiles",
            sa.Column("id", sa.String(36), primary_key=True),
    ...
    if not _has_table("org_settings"):
        op.create_table(
            "org_settings",
            sa.Column("id", sa.String(36), primary_key=True),
