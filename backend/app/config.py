...
class Settings(BaseSettings):
    ...
    # Security & Keys — REQUIRED from the environment (no fallbacks).
    # Production refuses to boot without them; non-production generates
    # ephemeral keys with a warning so local dev still runs.
    secret_key: str = Field(default="", validation_alias=AliasChoices("SECRET_KEY", "secret_key"))
    jwt_secret_key: str = Field(default="", validation_alias=AliasChoices("JWT_SECRET_KEY", "jwt_secret_key"))
    ...
    # Database — credentials come from the environment; DATABASE_URL is built
    # from POSTGRES_* parts when not set explicitly.
    postgres_host: str = Field(default="postgres", validation_alias=AliasChoices("POSTGRES_HOST", "postgres_host"))
    ...
    # Redis & Celery — password required from env; URLs built from parts.
    redis_host: str = Field(default="redis", validation_alias=AliasChoices("REDIS_HOST", "redis_host"))
    redis_port: int = Field(default=6379, validation_alias=AliasChoices("REDIS_PORT", "redis_port"))
    ...
    celery_broker_url: str = Field(default="", validation_alias=AliasChoices("CELERY_BROKER_URL", "celery_broker_url"))
    celery_result_backend: str = Field(default="", validation_alias=AliasChoices("CELERY_RESULT_BACKEND", "celery_result_backend"))
    ...
    openai_model_priority: str = Field(default="gpt-4o", validation_alias=AliasChoices("OPENAI_MODEL_PRIORITY", "openai_model_priority"))
    openai_api_base: str = Field(default="https://api.openai.com/v1", validation_alias=AliasChoices("OPENAI_API_BASE", "openai_api_base"))
    ...
    paddle_price_id_pro_plus_monthly: str = Field(default="", validation_alias=AliasChoices("PADDLE_PRICE_ID_PRO_PLUS_MONTHLY", "paddle_price_id_pro_plus_monthly"))
    paddle_price_id_pro_plus_annual: str = Field(default="", validation_alias=AliasChoices("PADDLE_PRICE_ID_PRO_PLUS_ANNUAL", "paddle_price_id_pro_plus_annual"))
    paddle_price_id_team_monthly: str = Field(default="", validation_alias=AliasChoices("PADDLE_PRICE_ID_TEAM_MONTHLY", "paddle_price_id_team_monthly"))
    paddle_price_id_team_annual: str = Field(default="", validation_alias=AliasChoices("PADDLE_PRICE_ID_TEAM_ANNUAL", "paddle_price_id_team_annual"))
    paddle_price_id_whatsapp_250: str = Field(default="", validation_alias=AliasChoices("PADDLE_PRICE_ID_WHATSAPP_250", "paddle_price_id_whatsapp_250"))
    paddle_price_id_whatsapp_500: str = Field(default="", validation_alias=AliasChoices("PADDLE_PRICE_ID_WHATSAPP_500", "paddle_price_id_whatsapp_500"))
    ...
    # Host-level reverse proxy target for the API (used by deploy docs/scripts)
    api_proxy_url: str = Field(default="http://127.0.0.1:8000", validation_alias=AliasChoices("API_PROXY_URL", "api_proxy_url"))
    ...
    def _resolve_secrets_and_urls(self):
        # 1. Compose composite URLs from parts when not set explicitly.
        ...
        if not self.redis_url:
            self.redis_url = f"redis://{redis_auth}{self.redis_host}:{self.redis_port}/0"
        ...
        if is_prod and missing:
            raise RuntimeError(
                ...
                + "then run backend/scripts/security_check.py."
            )
        if not is_prod:
            if not self.secret_key:
                self.secret_key = "dev-only-" + _secrets.token_urlsafe(32)
            if not self.jwt_secret_key:
                self.jwt_secret_key = "dev-only-" + _secrets.token_urlsafe(32)
        return self
